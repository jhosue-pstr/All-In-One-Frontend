...
export const blogService = {
  ...
  async deletePost(siteId: number, postId: number): Promise<{ message: string }> {
    return fetchApi<{ message: string }>(
      `/modules/blog/${siteId}/posts/${postId}`,
      {
        method: "DELETE",
      }
    );
  ...
  async uploadImage(siteId: number, file: File): Promise<BlogImageResponse> {
    ...
    const response = await fetch(
      `${API_URL}/modules/blog/${siteId}/upload-image`,
      {
        method: "POST",
        headers: {
